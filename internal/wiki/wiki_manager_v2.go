...
func (dm *DocumentManager) GenerateDocument(ctx context.Context, docType DocumentType, repoPath string) (*DocumentStructure, error) {
	...
	if dm.store != nil {
		...
		if err := dm.store.SaveDocument(repoPath, docStructure, string(docType)); err != nil {
			...
			// 获取存储路径用于调试
			if fileStore, ok := dm.store.(*EnhancedFileStore); ok {
...
// 返回: 错误信息
func (dm *DocumentManager) ExportDocument(repoPath, outputPath, format string, docType DocumentType, mode ...string) error {
	// 从 store 中加载文档结构
	if dm.store == nil {
	...
	switch format {
	case "markdown":
		return dm.exporter.ExportMarkdown(dm.convertToWikiStructure(docStructure), outputPath, markdownMode)
	case "json":
...
// mode: 导出模式，仅对 markdown 格式有效 ("single" 或 "multi")
// 返回: 错误信息
func (dm *DocumentManager) ExportWiki(repoPath, outputPath, format string, mode ...string) error {
	return dm.ExportDocument(repoPath, outputPath, format, DocTypeWiki, mode...)
}
...
// mode: 导出模式，仅对 markdown 格式有效 ("single" 或 "multi")
// 返回: 错误信息
func (dm *DocumentManager) ExportCodeRules(repoPath, outputPath, format string, mode ...string) error {
	return dm.ExportDocument(repoPath, outputPath, format, DocTypeCodeRules, mode...)
}
