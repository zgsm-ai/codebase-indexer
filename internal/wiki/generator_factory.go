...
func NewGeneratorFactory(log logger.Logger) *GeneratorFactory {
	...
	// 注册默认的生成器
	factory.RegisterGenerator(DocTypeWiki, NewWikiGeneratorV2)
...
// WikiGeneratorV2 兼容原版的Wiki生成器（基于新的基础架构）
type WikiGeneratorV2 struct {
	*BaseGenerator
}

// NewWikiGeneratorV2 创建Wiki生成器V2
func NewWikiGeneratorV2(config *SimpleConfig, logger logger.Logger) (DocumentGenerator, error) {
	baseGen, err := NewBaseGenerator(config, logger, DocTypeWiki)
	if err != nil {
		return nil, err
	}

	return &WikiGeneratorV2{
		BaseGenerator: baseGen,
	}, nil
}

// GenerateDocument 生成Wiki文档
func (g *WikiGeneratorV2) GenerateDocument(ctx context.Context, repoPath string) (*DocumentStructure, error) {
	return g.GenerateDocumentWithProgress(ctx, repoPath, LogProgressCallback)
}

// GenerateDocumentWithProgress 带进度回调的Wiki文档生成
func (g *WikiGeneratorV2) GenerateDocumentWithProgress(ctx context.Context, repoPath string, callback ProgressCallback) (*DocumentStructure, error) {
	// 执行通用生成逻辑
...
func (g *WikiGeneratorV2) selectRepresentativeFiles(fileMetas []*FileMeta) []*FileContent {
...
func (g *WikiGeneratorV2) generateProjectOverview(ctx context.Context, repoPath string, files []*FileContent) (*DocumentPage, error) {
	// 获取文件路径列表
...
func (g *WikiGeneratorV2) generateWikiStructure(ctx context.Context, fileTree string, readmeContent string, repoPath string, fileMetas []*FileMeta) (*DocumentStructure, error) {
	// 获取文件路径列表
	var filePaths []string
	for _, fileMeta := range fileMetas {
...
func (g *WikiGeneratorV2) getWikiStructurePrompt(fileTree string, readmeContent string, filePaths []string) (string, error) {
	outputLang := getOutputLanguage(g.config.Language)
...
func (g *WikiGeneratorV2) parseWikiStructureXML(xmlText string) (*DocumentStructure, error) {
	// 清理markdown分隔符
...
func (g *WikiGeneratorV2) parseWikiPageXML(xmlText string) *DocumentPage {
	page := &DocumentPage{
		Metadata: make(map[string]interface{}),
	}
...
func (g *WikiGeneratorV2) parseWikiSectionXML(xmlText string) *DocumentSection {
...
func (g *WikiGeneratorV2) categorizePagesIntoSections(pages []DocumentPage, overviewPage *DocumentPage, repoPath string) *DocumentStructure {
	// 定义常见类别
	categories := []struct {
		id       string
		title    string
...
func (g *WikiGeneratorV2) outputFinalStats(callback ProgressCallback) {
	g.performance.Finish()
