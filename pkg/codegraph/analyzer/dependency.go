...
func (da *DependencyAnalyzer) SaveSymbolOccurrences(ctx context.Context, projectUuid string,
	...
	updatedSymbolOccurrences := make([]*codegraphpb.SymbolOccurrence, 0)

	symbolOccurrencesMap := make(map[string]*codegraphpb.SymbolOccurrence)
	for _, fileTable := range fileElementTables {
		...
		for _, elem := range fileTable.Elements {
			...
			case *resolver.Class, *resolver.Function, *resolver.Method, *resolver.Variable, *resolver.Interface:
				...
				d := symbolOccurrencesMap[key]
				//if !ok {
				//	// TODO 同名处理
				//	// load
				//	nameKey := store.SymbolNameKey{Name: key, Language: fileTable.Language}
				//	exists, err := da.store.Exists(ctx, projectUuid, nameKey)
				//	if err == nil && exists {
				...
				//			da.logger.Debug("dependency_analyze get symbol occurrence err:%v", err)
				//		}
				//	}
				//
				//}
				if d == nil {
					d = &codegraphpb.SymbolOccurrence{Name: key, Language: string(fileTable.Language),
				...
				//	if o.Path != fileTable.Path {
				//		newOccurrences = append(newOccurrences, o)
				//	}
				//}
				d.Occurrences = append(d.Occurrences, &codegraphpb.Occurrence{
					Path:        fileTable.Path,
					Range:       elem.GetRange(),
					ElementType: proto.ElementTypeToProto(elem.GetType()),
				})
				symbolOccurrencesMap[key] = d
				updatedSymbolOccurrences = append(updatedSymbolOccurrences, d)
